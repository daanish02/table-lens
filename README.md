...
Backed by 80+ automated tests — structural validation of every generate-then-validate guard, API route coverage, and real database round-trips, not just mocked-out happy paths. A handful of genuine end-to-end tests that hit a live LLM are marked `slow` and run separately, so the everyday suite stays fast without losing real coverage.
